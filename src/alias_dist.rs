...
impl AliasDistribution {
    pub fn new(weights: Vec<f64>) -> Result<AliasDistribution, AliasDistributionError> {
        if weights.iter().any(|&w| w < 0.0) { return Err(AliasDistributionError::InvalidWeights); };
        let size = weights.len();
        let total = weights.iter().fold(0.0, |sum, x| sum + x);
        if total == 0.0 { return Err(AliasDistributionError::NullDistribution); };
        let mut entropy = 0.0;
...
mod tests {
    ...
    fn test_new() {
        ...
        for weights in test_cases {
            ...
            for (i, (&j, p)) in dist.alias_table.iter().zip(dist.probability_table).enumerate() {
                ps[i] += p / count;
                ps[j] += (1.0 - p) / count;
            }
